...
if len(results_df) > 0:
    ...
    # Get the predicted class index for the selected instance
    predicted_class_idx = pred_classes[instance_idx]
    ...
    # The result is a list of arrays (one per class), with shape (1, num_features)
    shap_values_for_plot = shap_values[predicted_class_idx][0] 
    ...
    st.write(f"SHAP Explanation for {predicted_class_label} (Row {instance_idx})")
    fig_shap = plot_shap_local(
        shap_values_for_plot, 
        selected_features, 
        title=f"SHAP Feature Contribution for {predicted_class_label}"
    )
    st.pyplot(fig_shap)
