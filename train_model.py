...
import pandas as pd
import numpy as np
import joblib # Use joblib for robust loading of sklearn objects
...
# Define artifact paths based on the structure above
ARTIFACTS_DIR = 'model_artifacts'
MODEL_PATH = os.path.join(ARTIFACTS_DIR, 'cnn_bilstm_model.h5')
SCALER_PATH = os.path.join(ARTIFACTS_DIR, 'minmax_scaler.joblib')
ENCODER_PATH = os.path.join(ARTIFACTS_DIR, 'label_encoder.joblib')
FEATURES_PATH = os.path.join(ARTIFACTS_DIR, 'feature_names.joblib')
...
# ----------------------------
# Load resources (model, scaler, encoder, features) - MODIFIED
# ----------------------------
...
def load_resources():
    try:
        ...
        # Load Scaler and Encoder using joblib
        ...
        st.error(f"Missing required file: {e}. Ensure all files are in the '{ARTIFACTS_DIR}' directory.")
        st.stop()
...
# ----------------------------
# Helper for explainers (Predicts 2D input reshaped to 3D)
# ----------------------------
...
# ----------------------------
# Explainers Caching (Uses predict_2d_to_3d)
# ----------------------------
...
def get_shap_explainer(data, labels):
    ...
    # Initialize the KernelExplainer with the 2D-to-3D prediction function
    ...
    if st.button(f"Process Next Batch ({BATCH_SIZE} rows)"):
        with st.spinner(f"Processing Batch {batch_index + 1}..."):
            ...
            try:
                ...
                # 2. Reshape & Predict (This replicates the crucial training step)
...
if st.session_state.current_results_df is not None:
    ...
    with tab2:
        ...
        if class_rows.empty:
            ...
            current_batch_indices = df_sel_all.loc[results_df.index].index.values
